...
&Scoped-define QUERY-STRING-BROWSE-8 FOR EACH Restaurante.PRODUCTO SHARE-LOCK, ~
      EACH Restaurante.STOCK WHERE STOCK.ID_PRODUCTO = PRODUCTO.ID_PRODUCTO SHARE-LOCK, ~
      EACH Restaurante.UNIDAD_MEDIDA WHERE UNIDAD_MEDIDA.ID_UNIDAD = PRODUCTO.ID_UNIDAD SHARE-LOCK ~
    BY STOCK.F_INGRESO DESCENDING ~
       BY STOCK.LOTE DESCENDING INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-8 OPEN QUERY BROWSE-8 FOR EACH Restaurante.PRODUCTO SHARE-LOCK, ~
      EACH Restaurante.STOCK WHERE STOCK.ID_PRODUCTO = PRODUCTO.ID_PRODUCTO SHARE-LOCK, ~
      EACH Restaurante.UNIDAD_MEDIDA WHERE UNIDAD_MEDIDA.ID_UNIDAD = PRODUCTO.ID_UNIDAD SHARE-LOCK ~
    BY STOCK.F_INGRESO DESCENDING ~
       BY STOCK.LOTE DESCENDING INDEXED-REPOSITION.
...
&Scoped-Define ENABLED-OBJECTS BUTTON-25 FILL-IN-35 BROWSE-8 BUTTON-1 ~
BUTTON-22 BUTTON-20 RECT-1 RECT-18 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-35 
...
DEFINE VARIABLE FILL-IN-35 AS CHARACTER FORMAT "X(256)":U 
     LABEL "Buscar" 
     VIEW-AS FILL-IN 
     SIZE 32.2 BY 1
     BGCOLOR 15  NO-UNDO.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 166 BY 19.86
         BGCOLOR 15  ROW-HEIGHT-CHARS .91 EXPANDABLE.
...
DEFINE FRAME Inventario-Frame
     ...
     FILL-IN-35 AT ROW 4.81 COL 17 COLON-ALIGNED
     BROWSE-8 AT ROW 6.24 COL 11
     BUTTON-1 AT ROW 9.1 COL 184.6
     BUTTON-22 AT ROW 12.95 COL 184.6
     BUTTON-20 AT ROW 16.95 COL 184.8
...
/* SETTINGS FOR DIALOG-BOX Inventario-Frame
                                                                        */
/* BROWSE-TAB BROWSE-8 FILL-IN-35 Inventario-Frame */
ASSIGN 
...
/* Query rebuild information for BROWSE BROWSE-8
     ...
     _OrdList          = "Restaurante.STOCK.F_INGRESO|no,Restaurante.STOCK.LOTE|no"
...
DO:
    RUN Nuevo_Producto.w(viniduser).
    {&OPEN-query-BROWSE-8}
END.
...
DO:
   ...
   IF vlogborrar = TRUE THEN DO:
       ROWID(stock).
       Insertar_Bitacora(2,viniduser,stock.id_stock,stock.cantidad).
       DELETE stock.
       {&OPEN-query-BROWSE-8}
   END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-35 Inventario-Frame
ON VALUE-CHANGED OF FILL-IN-35 IN FRAME Inventario-Frame /* Buscar */
DO:
    DEFINE VARIABLE vcharConsulta AS CHARACTER.
    DEFINE VARIABLE vhandSearch AS HANDLE.

    vhandSearch = (QUERY BROWSE-8:HANDLE).
    vcharConsulta = " for EACH Restaurante.PRODUCTO, EACH Restaurante.STOCK WHERE STOCK.ID_PRODUCTO = PRODUCTO.ID_PRODUCTO, EACH Restaurante.UNIDAD_MEDIDA WHERE UNIDAD_MEDIDA.ID_UNIDAD = PRODUCTO.ID_UNIDAD and BEGINS producto.descripcion'" + TRIM(FILL-IN-35:SCREEN-VALUE) + "'" .
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-25 FILL-IN-35 BROWSE-8 BUTTON-1 BUTTON-22 BUTTON-20 RECT-1 
         RECT-18 
      WITH FRAME Inventario-Frame.
